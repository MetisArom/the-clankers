...
class ActiveTripsViewModel(private val token: String) : ViewModel() {
    ...
    fun loadActiveTrips(userId: Int) {
        viewModelScope.launch {
            ...
            try {
                val trips = ApiClient.getActiveTrips(token, userId)
...
@Composable
fun useActiveTrips(token: String, userId: Int): State<List<Int>?> {
    val viewModel = remember { ActiveTripsViewModel(token) }
