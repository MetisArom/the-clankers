...
class FriendsTripsViewModel(private val token: String) : ViewModel() {
    ...
    fun loadActiveTrips(userId: Int) {
        viewModelScope.launch {
            ...
            try {
                val trips = ApiClient.getFriendsTrips(token, userId)
...
@Composable
fun useFriendsTrips(token: String, userId: Int): State<List<Int>?> {
    val viewModel = remember { FriendsTripsViewModel(token) }
